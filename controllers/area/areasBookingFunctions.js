...
function areasBooking(callback) {
    connection
    let sql = `SELECT area_booking_id, user.name as username, user.lastName, user.email, area.name, date, duration, reason, state_booking.description FROM area_Booking inner join user on area_Booking.user_id=user.user_id inner join area on area_Booking.area_id = area.area_id inner join state_booking on area_Booking.state_id = state_booking.state_id`
